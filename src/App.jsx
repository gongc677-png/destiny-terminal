  ...
  return (
    <div className="relative z-[1] px-4 py-7 pb-10 responsive-container">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-2 bg-card border border-border rounded-lg p-1 gap-1 md:gap-2">
          <TabsTrigger value="bazi"
            className="data-[state=active]:bg-muted data-[state=active]:text-gold data-[state=active]:shadow-[0_0_12px_rgba(201,163,88,0.1)] text-muted-foreground text-xs md:text-sm px-2 md:px-4 py-2">
            八字排盘
          </TabsTrigger>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'xlr' && (
          <motion.div key="xlr" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
            <div className="mb-5 rounded-2xl border border-border bg-card p-4 md:p-6 flex flex-col gap-3 responsive-container">
              ...
              <div className="relative flex items-center gap-2">
                ...
                <span className="text-[11px] md:text-[12px] text-muted-foreground">或输入触�?</span>
                <div className="flex-1 h-px bg-border" />
              </div>
              ...
              <div className="flex flex-col gap-2 md:gap-3">
                <div className="flex gap-1.5 md:gap-2">
                  ...
                  <input type="number" value={xlrNum2} onChange={e => setXlrNum2(e.target.value)}
                    placeholder="�?" className="min-w-0 flex-1 bg-input border border-border text-foreground h-9 md:h-10 rounded-lg text-center text-sm placeholder:text-star-faint focus:outline-none focus:border-accent" />
      ...
      <footer className="text-center pt-10 pb-5 mt-3 border-t border-border">
        <p className="text-xs md:text-sm text-star-faint">天命终端 · 信息已陈�?</p>
        <p className="mt-2 text-[10px] md:text-[11px] text-star-faint/60 italic">器成则藚，功成身退。但若有人以赤诚之心叩问天地——我仍会应�?</p>
      </footer>
    </div>
  );
}
